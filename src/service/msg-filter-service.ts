...
import * as msgFilter from './msg-filters'
...
async function getMsgReply(resArray: string | any[], { that, msg, name, contact, config, avatar, id, room }: { that: any; msg: any; contact?: any; name: any; config: any; avatar: any; id: any; room?: any }) {
  let msgArr = []
 ...
 */
async function filterFriendMsg(that: any, contact: { name: () => any; id: any; avatar: () => any }, msg: string) {
 ...
 * 2 初次添加好友
 */
async function filterRoomMsg(that: any, msg: any, name: any, id: any, avatar: any, room: any) {
  const config = await allConfig() // 获取配置信息
  const resArray = [
    { bool: msg === '', method: 'emptyMsg' },
    { bool: config.eventKeywords && config.eventKeywords.length > 0, method: 'eventMsg' },
...
export { filterFriendMsg, filterRoomMsg }
